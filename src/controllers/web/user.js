...
exports.visited = async (req, res) => {
	try {
		...
		//首先查询之前有无访问记录
		const sqlString =
			"SELECT id, name, portrait,last_login_time,visited_count FROM web_account WHERE identify=?";
		//更新ip地址
		...
		if (result && result.length > 0) {
			...
			const sqlString1 =
				"UPDATE web_account SET ip = ?, address = ?,agent = ?, visited_count = ? WHERE identify=?";
