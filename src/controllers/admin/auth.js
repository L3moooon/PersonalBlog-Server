...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (type == 'account') {
      ...
      if (!storedCode) {
        return res.status(400).json({ code: 0, msg: '验证码已过期，请重新获取' });
      }
      ...
      if (userResult.length === 0) {
        // 插入新用户
        const insertSql = 'INSERT INTO admin_account (account,name，password) VALUES (?,?,NULL)';
    ...
    await redisClient.set(
      `user:session:${token}`,
      JSON.stringify({ user_id, permission, loginTime: new Date().toISOString() }),
      { EX: 604800 } // 一周过期
    );
    //登录成功时获取ip地址
