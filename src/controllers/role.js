...
exports.assignPermission = async (req, res) => {
  try {
    ...
    if (Array.isArray(permission_ids) && permission_ids.length > 0) {
      const insertValues = permission_ids.map(permissionId => [id, permissionId]);
      const insertSql = 'INSERT INTO role_permission_relation (role_id, permission_id) VALUES ?';
      await query(insertSql, [insertValues]);
    }
    return res.send({ status: 1, message: '权限分配成功' })
  } catch (error) {
    return res.send({ code: 0, message: error.message })
  }
}
